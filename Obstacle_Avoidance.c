/**
 * Obstacle_Avoidance.c - Obstacle detection and avoidance with ultrasonic sensor and servo
 * control.
 */
...
void update_speed_and_distance(void)
{
    ...
    printf("SPEED CALC: dist_traveled=%.2fcm, time=%.3fs, speed=%.2fcm/s\n", distance_traveled,
           time_elapsed_s, g_current_speed_cm_s);
...
void servo_set_angle(float angle)
{
    ...
    float pulse_width_us =
        SERVO_MIN_PULSE_US + (angle / 180.0f) * (SERVO_MAX_PULSE_US - SERVO_MIN_PULSE_US);
...
void print_encoder_status(void)
{
    ...
    printf(
        "[ENCODER] Left: %ld pulses, %.1f cm, %.1f cm/s | Right: %ld pulses, %.1f cm, %.1f cm/s\n",
        left_pulses, left_distance, left_speed, right_pulses, right_distance, right_speed);
}
...
{
    float angle_C =
        fabsf((servo_angle_left + 15.0f) - (servo_angle_right - 15.0f)) * (float)M_PI / 180.0f;
    ...
    while (get_average_pulses() < target_pulses) {
        ...
        if (current_time - last_print_time >= 100) {
            printf("[TURN] Progress: %lu/%lu pulses (Time: %lums)\n", get_average_pulses(),
                   target_pulses, current_time - start_time);
...
bool check_if_object_still_there(void)
{
    ...
    printf("[CHECK] Scan complete: %d detections in %d positions\n", detection_count,
           (int)((SCAN_END_ANGLE - SCAN_START_ANGLE) / SCAN_STEP) + 1);
...
#define target_pulses 12
        ...
        if (object_still_there) {
            ...
            if (no_object_counter >= REQUIRED_NO_OBJECT_CHECKS && turns_finished < 2) {
                ...
                //     while (!line_detected && (to_ms_since_boot(get_absolute_time()) -
                //     line_search_start_time < MAX_SEARCH_TIME_MS)) {
                //     // Read IR sensor
                //     uint16_t ir_raw = ir_read_raw();
                //     int color_state = classify_colour(ir_raw);
        ...
        while (approach_active) {
            ...
            if (distance < 0) {
                ...
                if (scan_result.width_cm > 0) {
                    ...
                    char width_str[30];
                    snprintf(width_str, sizeof(width_str), "object width: %.1fcm",
                             scan_result.width_cm);
...
void line_follow_with_obstacle_avoidance(void)
{
    printf("\n=== COMBINED LINE FOLLOWING WITH OBSTACLE AVOIDANCE ===\n");
    printf(
        "Behavior: Line follow until object detected -> Avoid object -> Resume line following\n");
...
void avoid_obstacle_only(void)
{
    ...
    while (cycle_active) {
        ...
        if (scan_result.width_cm > 0) {
            ...
            char width_str[100];
            snprintf(width_str, sizeof(width_str), "OBSTACLE_WIDTH: %.1fcm", scan_result.width_cm);
