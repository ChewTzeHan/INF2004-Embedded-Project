/**
 * PID_Line_Follow.c - PID controller for line following using IR sensor feedback.
 */
...
void set_motor_speeds_pid(uint16_t ir_value)
{
    ...
    if (ir_value < WHITE_THRESHOLD) {
        ...
        if (white_ramp_factor > 1.0f)
            white_ramp_factor = 1.0f;
...
void follow_line_simple_with_params(float base_left_speed, float base_right_speed,
                                    float max_correction)
{
    ...
    static uint32_t last_print = 0;
    if (current_time - last_print > 100000) { // Print every 100ms
        printf("IR: %4u | Error: %4d | Correction: %6.2f | Speeds: L=%.2f R=%.2f | Custom params: "
               "baseL=%.1f baseR=%.1f maxCorr=%.1f\n",
               ir_value, SETPOINT - ir_value, correction, left_speed, right_speed, base_left_speed,
               base_right_speed, max_correction);
        last_print = current_time;
    }
