/**
 * barcode.c - Code 39 barcode scanner implementation using IR sensor timing analysis.
 */
...
static const code39_char_t code39_table[] = {
    {'A', "WNNNNWNNW", 10}, {'B', "NNWNNWNNW", 11}, {'C', "WNWNNWNNN", 12}, {'D', "NNNNWWNNW", 13},
    {'E', "WNNNWWNNN", 14}, {'F', "NNWNWWNNN", 15}, {'G', "NNNNNWWNW", 16}, {'H', "WNNNNWWNN", 17},
    {'I', "NNWNNWWNN", 18}, {'J', "NNNNWWWNN", 19}, {'K', "WNNNNNNWW", 20}, {'L', "NNWNNNNWW", 21},
    {'M', "WNWNNNNWN", 22}, {'N', "NNNNWNNWW", 23}, {'O', "WNNNWNNWN", 24}, {'P', "NNWNWNNWN", 25},
    {'Q', "NNNNNNWWW", 26}, {'R', "WNNNNNWWN", 27}, {'S', "NNWNNNWWN", 28}, {'T', "NNNNWNWWN", 29},
    {'U', "WWNNNNNWN", 30}, {'V', "NWWNNNNWN", 31}, {'W', "WWWNNNNNN", 32}, {'X', "NWNNWNNWN", 33},
...
void barcode_irq_init(void)
{
    ...
    gpio_set_irq_enabled_with_callback(
        RIGHT_IR_DIGITAL_PIN, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &barcode_gpio_isr);
...
static uint32_t estimate_narrow_us(const uint32_t* w, uint16_t n)
{
    ...
    if (median < 10000)
        return 10000; // Minimum 10ms
    if (median > 500000)
        return 500000; // Maximum 500ms
...
static width_t classify_width(uint32_t dur_us, uint32_t narrow_us)
{
    ...
    if (ratio < 3.0f)
        return WIDTH_NARROW; // 0.3x to 2.0x = Narrow
    if (ratio > 3.0f)
        return WIDTH_WIDE; // 2.0x to 5.0x = Wide
    return WIDTH_BAD;      // Too long
}

static bool decode_symbols_to_chars(const uint32_t* dur, uint16_t n, char* out, uint8_t* out_len,
...
static bool decode_with_direction(const uint32_t* dur, uint16_t n, barcode_result_t* result,
                                  bool forward)
{
...
bool barcode_scan_digital(barcode_result_t* result)
{
    ...
    if (!decode_with_direction(dur, n, result, true)) {
        if (!decode_with_direction(dur, n, result, false))
            return false;
    }
...
barcode_command_t barcode_parse_command(const char* s)
{
    ...
    if (!strcmp(s, "GO") || !strcmp(s, "F") || !strcmp(s, "FWD") || !strcmp(s, "FORWARD"))
...
static bool decode_with_intercharacter_gap(const uint32_t* dur, uint16_t n,
            ...
            if (c != '?') {
                ...
                if (valid_decodes_count == 1) {
                    ...
                    for (int char_pos = start_pos; char_pos <= n - 9; char_pos += 9) {
                        ...
                        if (char_valid) {
                            ...
                            if (decoded_char != '?') {
                                ...
                                if (str_index >= (int)sizeof(test_string) - 2)
                                    break;
                            } else {
                                break;
                            }
                    ...
                    if (str_index > 1) { // At least one character besides start marker
                        ...
                        if (validate_and_strip_code39(validated_chars, &validated_len,
                                                      &checksum_ok)) {
                            strncpy(result->data, validated_chars, BARCODE_MAX_LENGTH);
                            result->length = (uint8_t)strnlen(result->data, BARCODE_MAX_LENGTH);
                            result->valid  = (result->length > 0);
                               ...
                               size_t direction_size)
{
    ...
    for (int i = 0; i < n && i < 20 && timing_index < (timing_size - 10); i++) {
        ...
        timing_index +=
            snprintf(timing_str + timing_index, timing_size - timing_index, "%u", dur[i]);
    }
    timing_index += snprintf(timing_str + timing_index, timing_size - timing_index, "]");
    ...
    bool decode_success =
        decode_with_sliding_window(dur, n, result, all_decoded_values, sizeof(all_decoded_values));
    ...
    if (decode_success) {
        ...
        snprintf(state, sizeof(state),
                 "DECODED: %s | DIRECTION: %s | %s | PATTERN: %s | TIMING: %s", result->data,
                 direction_str, all_decoded_values, nw_pattern, timing_str);
...
void barcode_update_speed_and_distance(void)
{
    ...
    printf("[BARCODE_SPEED] dist_traveled=%.2fcm, time=%.3fs, speed=%.2fcm/s\n", distance_traveled,
           time_elapsed_s, g_barcode_current_speed_cm_s);
...
static void barcode_detection_task(void* pv)
{
    ...
    while (system_active) {
        ...
        if (current_state && !scanning_active &&
            !junction_complete) {  // Black detected and not already scanning
        ...
        if (scanning_active) {
            ...
            if (barcode_scan_while_moving(&scan_result, nw_pattern, sizeof(nw_pattern), timing_str,
                                          sizeof(timing_str), direction_str,
                                          sizeof(direction_str))) {
                printf("\n🎯 BARCODE SCANNED SUCCESSFULLY WHILE MOVING!\n");
                printf("Data: '%s' (Length: %u, Checksum: %s)\n", scan_result.data,
                       scan_result.length, scan_result.checksum_ok ? "OK" : "FAIL");
                printf("Scan time: %u us\n", scan_result.scan_time_us);
                printf("N/W Pattern: %s\n", nw_pattern);
                printf("Timing Array: %s\n", timing_str);
                printf("Inferred Direction: %s\n", direction_str);

                // Send telemetry with decoded barcode data, timing information, AND inferred
                // direction
                ...
                if (mqtt_is_connected() && (now - g_last_pub_ms >= 500)) {
                    ...
                    char state[400];
                    snprintf(state, sizeof(state), "SCAN_FAILED | PATTERN: %s | TIMING: %s",
                             nw_pattern, timing_str);
            ...
            if (current_time - scan_start_time > SCAN_TIMEOUT_MS) {
                ...
                scan_result.scan_time_us =
                    (current_time - scan_start_time) * 1000; // Convert ms to us
                ...
                if (mqtt_is_connected() && (current_time - g_last_pub_ms >= 500)) {
                    ...
                    char state[400];
                    snprintf(state, sizeof(state), "TIMEOUT | PATTERN: %s | TIMING: %s", nw_pattern,
                             timing_str);
                    float speed    = barcode_get_current_speed_cm_s();
                    float distance = barcode_get_total_distance_cm();
                    mqtt_publish_telemetry(speed, distance, 0.0f, 0.0f, state);
