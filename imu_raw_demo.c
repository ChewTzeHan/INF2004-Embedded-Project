/**
 * imu_raw_demo.c - Raw IMU sensor demonstration and data acquisition.
 */
...
static mag_calibration_t mag_cal = {.offset_x = 10.0f,
                                    .offset_y = -132.0f,
                                    .offset_z = -200.0f,
                                    .scale_x  = 341.2f,
                                    .scale_y  = 341.2f,
...
bool read_accel_raw(int16_t* raw_ax, int16_t* raw_ay, int16_t* raw_az)
{
    ...
    if (i2c_write_blocking(I2C_INST, ACC_ADDR, &reg, 1, true) != 1)
        return false;
    if (i2c_read_blocking(I2C_INST, ACC_ADDR, acc_data, 6, false) != 6)
        return false;
...
bool read_mag_raw(int16_t* mx, int16_t* my, int16_t* mz)
{
    ...
    if (i2c_write_blocking(I2C_INST, MAG_ADDR, &reg, 1, true) != 1)
        return false;
    if (i2c_read_blocking(I2C_INST, MAG_ADDR, mag_data, 6, false) != 6)
        return false;
...
float get_heading_fast(float* direction_str)
{
    ...
    mag_samples_count =
        (mag_samples_count < MAG_FILTER_SIZE) ? mag_samples_count + 1 : MAG_FILTER_SIZE;
...
void simple_calibration()
{
    ...
    while (absolute_time_diff_us(start, get_absolute_time()) < 30000000) {
        ...
        if (read_mag_raw(&mx, &my, &mz)) {
            ...
            if (samples % 10 == 0) {
                printf("X[%5d,%5d] Y[%5d,%5d] Z[%5d,%5d] N=%d\r", min_x, max_x, min_y, max_y, min_z,
                       max_z, samples);
            }
        }
        sleep_ms(50);
