/**
 * ir_sensor.c - IR sensor driver for line detection and analog/digital readings.
 */
...
int ir_classify_hysteresis(uint16_t sample, const ir_calib_t* cal, int prev_state)
{
    ...
    if (sample <= low)
        return 1; // definitely black
    if (sample >= high)
        return 0; // definitely white
    return prev_state;
#else
    // Inverted behaviour
    if (sample >= high)
        return 1; // definitely black (higher = black)
    if (sample <= low)
        return 0; // definitely white
    return prev_state;
#endif
...
void test_digital_ir_interrupts(void)
{
    ...
    gpio_set_irq_enabled_with_callback(
        RIGHT_IR_DIGITAL_PIN, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &digital_ir_test_isr);
    ...
    while (true) {
        ...
        if (now - last_status > 2000000) {
            printf("Status: %lu IRQs in %lu ms (current state: %d)\n", g_irq_count,
                   (now - start_time) / 1000, ir_read_digital());
            last_status = now;
        }
    }
