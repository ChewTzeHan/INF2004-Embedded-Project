/**
 * main.c - Main application coordinating robot state machine with line following, obstacle
 * avoidance, and barcode scanning.
 */
...
static void robot_control_task(void* pv)
{
    ...
    while (system_active) {
        ...
        if (now - last_speed_update_time >= 100) {
            update_speed_and_distance(); // This updates g_current_speed_cm_s and
                                         // g_total_distance_cm
        ...
        if (mqtt_is_connected() && (now - last_telemetry_time >= 2000)) {
            ...
            case STATE_LINE_FOLLOWING:
                state_str = "LINE_FOLLOWING";
                break;
            case STATE_OBSTACLE_AVOIDANCE:
                state_str = "OBSTACLE_AVOIDANCE";
                break;
            case STATE_BARCODE_SCANNING:
                state_str = "BARCODE_SCANNING";
                break;
            case STATE_WAITING_FOR_JUNCTION:
                state_str = "WAITING_FOR_JUNCTION";
                break;
            case STATE_EXECUTING_TURN:
                state_str = "EXECUTING_TURN";
                break;
            }

            // Debug output to verify values
            printf("[TELEMETRY] Speed: %.2f cm/s, Distance: %.2f cm, Yaw: %.2f°, Ultra: %.2f cm, "
                   "State: %s\n",
        ...
        case STATE_BARCODE_SCANNING: {
            ...
            if (barcode_scan_only(&scan_result, nw_pattern, sizeof(nw_pattern), timing_str,
                                  sizeof(timing_str), direction_str, sizeof(direction_str))) {
                ...
                snprintf(result_msg, sizeof(result_msg),
                         "BARCODE_SCANNED: %s -> %s (Waiting for junction)", scan_result.data,
                         direction_str);
                ...
                char result_msg[100];
                snprintf(result_msg, sizeof(result_msg),
                         "BARCODE_SCANNED: C -> RIGHT (Waiting for junction)");
                mqtt_publish_telemetry(speed, distance, yaw, ultra, result_msg);
            }
        } break;
